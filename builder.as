...
function initializeBuilder() {
	// set up clipArray
	startScreen0.duplicateMovieClip("startScreen");
	startScreen0._visible = false;
...
//		trace("tempObj.youngerSiblingFalse = " + tempObj.youngerSiblingFalse)
		tempObj.xBranchTrueOffset =0;
		var weirdAdjustHackOffset = (tempBranchClip.nestingLevel == 0 )? tempBranchClip._width : tempBranchClip._width*4/(2-tempBranchClip.nestingLevel);
		if (tempObj.youngerSiblingTrue !== false) {
			var tempBranchClip = tempObj.youngerSiblingTrue;
			if (!doMoves) {
//				tempObjtempNestingLevel = tempObj.nestingLevel +1;
			}
			counter = 0;
			do {
...
//				tempBranchClip.nestingLevel = tempObj.nestingLevel;
				var weirdAdjustHackOffset = (tempBranchClip.nestingLevel == 0 || tempBranchClip.nestingLevel == undefined || isNaN(tempBranchClip.nestingLevel) )? tempBranchClip._width : tempBranchClip._width*4/(2-tempBranchClip.nestingLevel)
				weirdAdjustHackOffset = tempBranchClip._width*2;
				tempSizeBranch = check_branch_layout(tempBranchClip,tempObj, "check_branch_layout true") + ((tempBranchClip.clipType == "branch") ? weirdAdjustHackOffset : 0 );
//				tempSize = (tempBranchClip.clipType == "branch") ? 0 : tempSize;
...
//				tempObjtempNestingLevel = tempObj.nestingLevel +1
			...
			do {
				...
				var weirdAdjustHackOffset = (tempBranchClip.nestingLevel == 0 || tempBranchClip.nestingLevel == undefined || isNaN(tempBranchClip.nestingLevel) )? tempBranchClip._width : tempBranchClip._width*4/(2-tempBranchClip.nestingLevel);
				weirdAdjustHackOffset = tempBranchClip._width*2;
				trace("weirdAdjustHackOffset = " + weirdAdjustHackOffset);
				tempSizeBranch = check_branch_layout(tempBranchClip,tempObj, "check_branch_layout false") + ((tempBranchClip.clipType == "branch") ? weirdAdjustHackOffset : 0 );		
...
//		tempObj.nestingLevel = tempObjtempNestingLevel;	
//		tempObj.inBranch.nestingLevel ++;

		// IBBITY
		// This is a hack to try to get the _x values down.
		// SOmething's being overcounted somewhere, but I'm unsure where or how.
		// If the hack works, the fuck it. go with it.
		tempObj.xBranchFalseOffset = tempObj.xBranchFalseOffset - (nestingOffset * 2 *(tempObj.nestingLevel));
		tempObj.xBranchTrueOffset = tempObj.xBranchTrueOffset - (nestingOffset * 2 *(tempObj.nestingLevel));	
...
//	trace("\nlaying out tempScreen = " + tempScreen + "  and youngerSibling= " + tempScreen.youngerSibling);
...
//			trace("tempScreen.inBranch.nestingLevel = " + tempScreen.inBranch.nestingLevel);
			...
			if (tempScreen != currentDragClip) {
				...
				tempYOffset = (tempScreen.inBranch == tempScreen.olderSibling || tempScreen.olderSibling == currentDragClip) ? ((tempScreen.inBranch !== false ) ? (tempScreen.nestingSide * tempScreen.inBranch.nestingLevel * baseBranchVertical* currentScale/100) + (nestingOffset * tempScreen.nestingSide) :0) : 0;
		 		// nestingOffset + branchYCentering - (tempBranch.nestingLevel * baseBranchVertical* currentScale/100);
				// actual moves
	//			trace("tempScreen.olderSibling._x = " + tempScreen.olderSibling._x);
	//			trace("tempScreen.olderSibling._width = " + tempScreen.olderSibling._width);
	//			trace("tempXOffset = " + tempXOffset);
				tempScreen._x = (tempScreen.olderSibling != currentDragClip) ? tempScreen.olderSibling._x + tempScreen.olderSibling._width + tempXOffset : tempScreen.olderSibling.olderSibling._x + ((tempScreen.olderSibling.olderSibling.clipType == "branch") ? tempScreen.olderSibling.olderSibling.pathOffset + tempScreen.olderSibling.olderSibling._width : tempScreen.olderSibling.olderSibling._width) + tempScreen.olderSibling._width - xClipOverhang + tempXOffset;
				tempScreen._y = (tempScreen.olderSibling != currentDragClip) ? tempScreen.olderSibling._y + tempYOffset : ((tempScreen.olderSibling.olderSibling.inBranch != tempScreen.inBranch) ? tempScreen.olderSibling.olderSibling._y + tempYOffset  : tempScreen.olderSibling.olderSibling._y);
			}
		} else {
		...
		if (tempScreen.clipType == "branch") {
			...
			sizeBranchPaths(tempScreen,true,false,0);
		}
...
// IBBITY:
// May need this do/while loop, may not.
if (!doMoves)	{
	branchDepthCounter =0;
}
var counter =0;
...
//		trace("overrideNestCount = " + overrideNestCount);
...
//		trace("tempBranch.nestingLevel*baseBranchVertical = " + tempBranch.nestingLevel*baseBranchVertical);
		...
		trace(tempBranch+".xBranchTrueOffset = " + tempBranch.xBranchTrueOffset);
		trace(tempBranch+".xBranchFalseOffset = " + tempBranch.xBranchFalseOffset);
...
//		trace("nestingOffset = " + nestingOffset);
	...
	if (doMoves == false) {
		if (tempBranch.youngerSiblingTrue != false) {
			branchDepthCounter ++;						
		}
		if (tempBranch.youngerSiblingFalse != false) {
			branchDepthCounter ++;
		}
	...
	tempObj = tempBranch;
	isBranch = false;
 } while (tempBranch.inBranch != false && tempBranch.inBranch != undefined) 

branchDepthCounter = branchDepthCounter - counter;
...
function zoom ( scale, skipSlider, skipLayout ) {
				...
				switch (hitsArr.length) {
					...
					case 1: 
						...
						if (tempClip.clipType == "branchClose") {
							trace("Closing Branch Condition")
							tempBranch = tempClip.olderSibling;
							trace("tempBranch = " + tempBranch);
							trace("tempClip = " + tempClip);
							trace("clip = " + clip);
							trace("clip.youngerSibling = " + clip.youngerSibling);
							trace("clip.olderSibling = " + clip.olderSibling);
							...
							trace("clip.youngerSibling = " + clip.youngerSibling);
							trace("clip.olderSibling = " + clip.olderSibling);
							trace(clip.youngerSibling +".olderSibling = " + clip.youngerSibling.olderSibling);
						} else {
		...
		for (k=0; k<currentEditScreen.modules.length; k++) {
			...
			if (loadedClip.name == "widget_text") {
				if (currentEditScreen.modules[k].modified != true) {
					loadedClip.params.splice(1,0,new parameter ("Font", "Helvetica", "const", "string", "The name of the font you would like to use."));
					loadedClip.params.splice(1,0,new parameter ("Size", "18", "const", "integer", "The font size you would like to use."));
					loadedClip.params.splice(1,0,new parameter ("Text", "Sample Text\nSample Text\nSample Text", "const", "cdata", "The text to display. This will update the textValue field below."));
				}
...
function createScreenXML (tempScreen) {
	...
	for (j=0; j<tempScreen.modules.length; j++) {
		...
		for (k=0; k<currMod.params.length; k++) {
			if (currMod.name != "widget_text" || ( currMod.name == "widget_text" && currMod.params[k].name != "Text" && currMod.params[k].name != "Font" && currMod.params[k].name != "Size" )) {
				var currParam = currMod.params[k];
				if (currParam.name == "internalName") {
					if (typeof(currParam.value) != "undefined" && currParam.value != undefined && currParam.value != "undefined") 		{ xmlText += " name=\"" + 	currParam.value+ "\""; }
				} else {
					postTagXMLText += "\t\t\t<" + ((currParam.mode=="variable")? "field": currParam.mode);
		 			postTagXMLText += " type=\"" + currParam.type + "\"";
		 			postTagXMLText +=  (typeof(currParam.name) != "undefined" && currParam.name != undefined && currParam.name !== "undefined") 	 ? " name=\"" + currParam.name + "\"" : "";
					if(currParam.value != null ) {
						if (currParam.type == "cdata") {
							postTagXMLText += "><![CDATA[" + currParam.value + "]]>"
						} else {
							postTagXMLText += " value=\"" + currParam.value +   "\">"		
						}				
					} else {
						postTagXMLText += ">";
					}
...
/*								trace("size = " + size);
								trace("face = " + face);
								trace("text = " + text);
*/								tempModule.params.splice(0,0,new parameter ("Font", face, "const", "string", "The name of the font you would like to use."));
								tempModule.params.splice(0,0,new parameter ("Size", size, "const", "integer", "The font size you would like to use."));
								tempModule.params.splice(0,0,new parameter ("Text", text, "const", "cdata", "The text to display. This will update the textValue field below."));
							} else {
/*								var attribValue = tempChild.attributes.value;									
	...
	//						trace("attribValue = " + attribValue);
				...
				if (tempModule.modified === true) {
					tempModule.params.push(new parameter ("textValue", textString, "const", "cdata", "The textValue - this is what will actually be stored in the XML."));
				}
